...
export class CoinCard extends React.Component<Props> {
    componentDidMount() {
        const target = document.querySelector(`#coin_${this.props.coin.CoinInfo.Id}`) as HTMLElement
        setTimeout(() => {
    ...
    render() {
        ...
        const CardHeader = styled.div`
            ...
            align-items: center;
            height: 60px;
            border-bottom: 1px solid ${theme.card_line_color};
        `;
        ...
        const Price = styled.p`
            ...
            font-weight: 300;
            margin: 15px;
            color: ${theme.disabled_font_color};
        ...
        return (
            <Container>
                <Card id={`coin_${coin.CoinInfo.Id}`}>
                    <CardHeader>
                        <Title>{`${index + 1}. ${coin.CoinInfo.FullName}`}</Title>
                        <Image src={`https://cryptocompare.com${coin.CoinInfo.ImageUrl}`} />
                    </CardHeader>
                    <CardBody>
                        <Price>
                        ...
                        <CoinInfo>
                            {'24H Volume: '} <span>{coin.DISPLAY.USD.VOLUMEDAY}</span>
                        </CoinInfo>
                        <CoinInfo>
                            {'Market Cap: '} <span>{coin.DISPLAY.USD.MKTCAP}</span>
                        </CoinInfo>
                    </CardBody>
                </Card>
            </Container>
